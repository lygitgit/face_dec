    ...
    def inference(self, image, detections):
        ...
        if self.model_name == 'faceReID_FR' or self.model_name == 'faceReID_OF':
            ...
            for d in range(detections.shape[0]):
                x1, y1, x2, y2 = detections[d, :4].astype(int)
                x1 = max(0, x1)
                y1 = max(0, y1)
                x2 = min(W - 1, x2)
                y2 = min(H - 1, y2)
                # 转为 (top, right, bottom, left)
        ...
        for patches in batch_patches:
            ...
            pred = self.model.extract(patches)
            ...
            if np.isnan(feat).any():
                for n in range(np.size(nans)):
                    if nans[n]:
                        ...
                        pred_ = self.model.extract(patch_np_)
